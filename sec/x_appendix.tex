...
\paragraph{Anonymous supplementary material.}
The anonymous supplementary material is available at
\url{https://osf.io/rc3hd/?view_only=d5754078e7c642459cda6d95c8d7b421}
and is organized into two parts. The \texttt{demo} folder contains seven
real-world manipulation videos, with long inactive periods removed and the
remaining footage shown at three times the recorded speed. The
\texttt{code} folder provides a review-oriented implementation snapshot of
the core method, covering OST-Map representation and construction, structural
and semantic scene encoding, relation-aware map reasoning, and the
transition-guided prediction model. The snapshot omits datasets, model
checkpoints, data-collection infrastructure, baseline implementations, and
...
the downstream policy on the simulation benchmarks. Ground-truth object masks,
6-DoF poses, geometric parameters, and persistent object identities are read
from the corresponding simulator states and task definitions. These quantities
are used only for evaluation and are not provided to the constructor.
Object extraction is measured by mask mean intersection over union (mIoU).
Pose accuracy is reported as the mean translation and rotation error of
matched task objects. Structural accuracy is measured by the normalized mean
absolute error (NMAE) of the predicted primitive dimensions, where each error
is normalized by the corresponding simulator-provided dimension. Temporal
identity accuracy measures the fraction of tracked object and structural-node
identities that remain consistent over the full observation window.
...
$\tau_{\theta}=10\%$.
We do not report these constructor metrics for the real-world experiments,
where the required dense masks, exact object poses, and geometric parameters
are not available as independent ground truth.
...
\begin{table*}[t]
    ...
    \begin{tabular}{lcccccc}
        ...
        \midrule
        Simulation average
        & \textbf{89.2} & \textbf{0.96} & \textbf{3.7}
        & \textbf{5.2} & \textbf{95.4} & \textbf{86.2} \\
        \bottomrule
    \end{tabular}
    \end{adjustbox}
\end{table*}
...
\paragraph{Analysis.}
The constructor recovers accurate and temporally stable OST-Maps across all
three simulation benchmarks. It achieves an average mask mIoU of $89.2\%$,
while the mean pose errors remain below one centimeter in translation and
$4^\circ$ in rotation. The $5.2\%$ structural NMAE further shows that the
estimated primitive dimensions closely match the simulator geometry. These
accuracies are important because OST-Map represents object state through both
pose and fine-grained structure; small errors in either component would
directly perturb the transition targets and spatial relations supplied to the
policy.

Identity accuracy reaches $95.4\%$, indicating that the same objects and
structural parts are consistently aligned across the observation history.
Under the stricter complete-map criterion, which requires every task-relevant
element in a frame to satisfy all extraction, pose, structure, and identity
conditions simultaneously, the constructor remains correct in $86.2\%$ of
cases. The relatively small gap between the individual metrics and this
all-or-nothing measure indicates that constructor errors are not dominated by
frequent catastrophic map failures. Overall, the results show that OSTRA
receives a reliable structured representation on which to model object-state
transitions.
